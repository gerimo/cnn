class PostsController < ApplicationController
  ...
  # GET /posts.json
  def index
    if params[:fanpage_id].present?
      @fanpage = Fanpage.find(params[:fanpage_id])
      @posts = @fanpage.posts.order(sort_column + " " + sort_direction).paginate(:page => params[:page], :per_page => 50)
    else
